...
document.body.addEventListener("keydown", (e) => {
  if (e.key == "s") {
    if(start == 0){
      start = 1;
      for (let i = 0; i < 20; i++) {
        if (rand(0, 100) <= drivers[i].chance) {
          if (drivers[i].speed < 60) 
          {
            drivers[i].speed = random(45, 70);
          }
        } else {
          drivers[i].speed = random(45, 55);
        }
      }
      requestAnimationFrame(frame);
    }
...
let timer1 = setInterval(() => {
  for (let i = 0; i < 20; i++) {
    if (drivers[i].drs != 1) {
      if (rand(0, 100) <= drivers[i].chance) {
        drivers[i].speed = random(60, 71);
      } else {
        drivers[i].speed = random(58, 68);
      }
    } else {
      if (rand(0, 100) <= drivers[i].chance) {
          drivers[i].speed = random(80,100);
        
          drivers[i].drs = 0;
      }else{
         drivers[i].speed = random(80,95);
...
function speedPopUpHandler() {
  ...
  let inv = setInterval(function () {
    dSpeed = Math.ceil(driver.speed * random(100, 150) * (random(13, 18) * 11) + 50);
